    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
            .cors().and().csrf().disable()
                ...
                .antMatchers(HttpMethod.GET, "/static/**").permitAll() // FIXME tylko po ssl'u (także / i /bundle.js i /)
                .antMatchers(HttpMethod.GET, "/api/public").permitAll()
                .antMatchers(HttpMethod.GET, "/api/user/current").permitAll()
                .antMatchers(HttpMethod.POST, "/api/user/login").permitAll()
                .antMatchers(HttpMethod.POST, "/api/user/register").permitAll()
                .antMatchers(HttpMethod.POST, "/api/**").hasAuthority(UserRoles.USER_AUTHORITY.getAuthority())
                .antMatchers(HttpMethod.GET, "/api/**").hasAuthority(UserRoles.USER_AUTHORITY.getAuthority())
                .antMatchers(HttpMethod.DELETE, "/api/**").hasAuthority(UserRoles.USER_AUTHORITY.getAuthority())
                .antMatchers(HttpMethod.PUT, "/api/**").hasAuthority(UserRoles.USER_AUTHORITY.getAuthority())
                .and()
